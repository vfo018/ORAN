...
from pyasn1.type import univ, namedtype, char, namedval

# 定义ASN.1结构

# 简化的E2NodeComponentConfigItem结构
...
def receive_e2_setup(encoded_response):
    try:
        # 这里假设encoded_response是一个包含多个E2NodeComponentConfigItem的序列
...
def create_subscription_request(req_id, ran_func_id, event_trigger, action_id, action_type, action_definition):
    ...
    subscription_request.setComponentByName('actions', [action_item])
...
def main():
    ...
    encoded_e2_setup_response = b'...'  # 这里替换为实际的编码消息
    e2_node_comp_config_add_list = receive_e2_setup(encoded_e2_setup_response)
    
    if e2_node_comp_config_add_list is None:
        print("Failed to decode E2 Setup response")
        return
    ...
    for cell_config in e2_node_comp_config_add_list:
        ...
        # 通过HTTP发送订阅请求
        ric_api_url = "http://localhost:8000/api/v1/kpm_subscription"  # 使用本地测试服务器地址
