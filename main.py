...
def main():
    ...
    try:
        df.to_csv('output_handover_scenario.csv', index=False)
        ...
        for cell in cells:
            ...
            for cat in cell.cat_list:
                for pe in cat.pe_list:
                    ...
                    data['cell_barring'].append(cell.cell_barring)
                    if has_anomaly == 1 or pe.load < PRB / PE_NUMBER:
                        traffic_steering = 1
    ...
    for x in range(CELL_NUMBER):
        ...
        for y in range(CAT_NUMBER):
            ...
            for z in range(PE_NUMBER):
                pe = PersonalEquipment(z + x * PE_NUMBER, pe_load_dict[z + x * PE_NUMBER], x)
...
def handover_scenario(pe, cells, outflow_settings):
    ...
    if source_cell:
        ...
        if target_cell:
            if pe.traffic_steering == 1 or source_cell.cell_radius < pe.cell_dist:
